...
class MTLFace(object):
    ...
    def parser():
        ...
        # LOSS
        parser.add_argument(
            "--fr_age_loss_weight", help="age loss weight", type=float, default=0.0
        )
        parser.add_argument(
            "--fr_da_loss_weight",
            help="cross age domain adaption loss weight",
            type=float,
            default=0.0,
        )
        ...
        parser.add_argument(
            "--milestone", help="milestones", type=int, nargs="*", default=[20, 40, 60]
        )
        parser.add_argument(
            "--warmup", help="learning rate warmup epoch", type=int, default=5
        )
        parser.add_argument(
            "--learning_rate", help="learning-rate", type=float, default=0.1
        )
        ...
        parser.add_argument(
            "--val_interval",
            help="val dataset interval iteration",
            type=int,
            default=1000,
        )
        ...
        parser.add_argument(
            "--local_rank",
            "--local-rank",
            help="local process rank, not need to be set.",
            default=0,
            type=int,
        )
        ...
        parser.add_argument(
            "--age_pretrained_path", help="age_pretrained_path", type=str
        )
    ...
    def fit(self):
        ...
        for n_iter in tqdm.trange(
            opt.restore_iter + 1, opt.num_iter + 1, disable=(opt.local_rank != 0)
        ):
            ...
            if opt.train_fas:
                ...
                _fas_inputs = [
                    self.fr.backbone.module,
                    self.fr.estimation_network,
                    fr_inputs[0],
                    fas_inputs[0],
                    fr_inputs[1],
                    fas_inputs[1],
